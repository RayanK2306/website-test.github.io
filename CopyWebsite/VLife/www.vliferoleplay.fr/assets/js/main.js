...
!(function($) {
  "use strict";
   ...
   //Shorten name
    $('#dropdownMenuButton').each(function() {
    var $pTag = $(this).find('r');
    if($pTag.text().length > 10){
        var shortText = $pTag.text();
        shortText = shortText.substring(0, 10);
        $pTag.addClass('fullArticle').hide();
        shortText += '...';
        $(this).append('<r class="preview">'+shortText+'</r>');
    }
	});	
  //Carousel for team
    ...
    if (location.pathname.replace(/^\//, '') == this.pathname.replace(/^\//, '') && location.hostname == this.hostname) {
      ...
      if (target.length) {
        ...
        if ($('body').hasClass('mobile-nav-active')) {
          ...
          $('.mobile-nav-toggle i').toggleClass('fas fa-bars fas fa-cross');
